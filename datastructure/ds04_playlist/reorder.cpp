...
namespace CP
{
  ...
  public:
    ...
    void reorder(int pos, std::vector<int> selected)
    {
      ...
      std::vector<iterator> insertIt;
      for (size_t i = 0; i < pos; i++)
        insertPos++;

      size_t readIdx = 0;
      iterator it = begin();
      for (size_t i = 0; i < mSize; i++, it++)
      {
        if (readIdx >= selected.size())
          break;
        if (i == selected[readIdx])
        {
          insertIt.push_back(it);
          readIdx++;
        }
      }

      for (size_t i = 0; i < insertIt.size(); i++)
      {
        insert(insertPos,*insertIt[i]);
        erase(insertIt[i]);
      }
    }
  };
